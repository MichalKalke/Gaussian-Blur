...
namespace Gauss
{
    ...
    {
#if DEBUG
        [DllImport(@"..\..\..\..\..\..\GaussianBlur\x64\Debug\JaAsm.dll")]

        static extern int GaussBoxes(int sigma, int size, int[] tab,int n); 
#else
        [DllImport(@"..\..\..\..\..\..\GaussianBlur\x64\Release\JaAsm.dll")]

        static extern int GaussBoxes(int sigma, int size, int[] tab, int n);
#endif
        public int threads = GaussBlur.threadsNumber;
        ...
        public GaussianBlur(Bitmap image)
        {
            ...
            Parallel.For(0, source.Length, new ParallelOptions { MaxDegreeOfParallelism = (int)threads }, i =>
            {
        ...
        public Bitmap Process(int radial)
        {
            ...
            Parallel.For(0, dest.Length, new ParallelOptions { MaxDegreeOfParallelism = (int)threads },  i =>
            {
        ...
        private void gaussBlur_4(int[] source, int[] dest, int r)
        {
            int matrix = 3;
            int tabSize = 3;
            if (GaussBlur.currectCheckBox == "C#")
            {
                BoxesForGauss gauss = new BoxesForGauss();
                var bxs = boxesForGauss(r, matrix);
                for(int i = 0; i <3; i++)
                {
                    Console.WriteLine(bxs[i]);
                }
            ...
            {
                int[] tab = { 0,0,0 };
                var bxs = GaussBoxes(r, matrix, tab, tabSize);
                for (int i = 0; i < 3; i++)
                {
                    Console.WriteLine(tab[i]);
                }
                boxBlur_4(source, dest, _width, _height, (tab[0] - 1) / 2);
                boxBlur_4(dest, source, _width, _height, (tab[1] - 1) / 2);
                boxBlur_4(source, dest, _width, _height, (tab[2] - 1) / 2);
            }
        ...
        private void boxBlurH_4(int[] source, int[] dest, int w, int h, int r)
        {
            ...
            Parallel.For(0, h, new ParallelOptions { MaxDegreeOfParallelism = (int)threads }, i =>
            {
                var ti = i * w;
                var li = ti;
                var ri = ti + r;
        ...
        {
            var iar = (double)1 / (r + r + 1);
            Parallel.For(0, w, new ParallelOptions { MaxDegreeOfParallelism = (int)threads }, i =>
            {
                var ti = i;
                var li = ti;
                var ri = ti + r * w;
...
namespace SuperfastBlur.BlurTester
{
    class Program
    {
        ...
        static void Main()
        {
            ///* var liczka[] =*/ MyProc1(3, 4);
            Application.EnableVisualStyles();
            Application.SetCompatibleTextRenderingDefault(false);
            Application.Run(new global::GaussianBlur.GaussBlur());
        }
    }
}
